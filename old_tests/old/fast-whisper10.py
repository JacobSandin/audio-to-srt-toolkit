    ...
    for segment in segments:
        for word in segment.words:
            ...
            if should_flush:
                ...
                # 🔧 Correct misaligned short segments
                if raw_duration > 15.0 and word_count <= 3:
                    print(f"⚠️  Misaligned short segment ({word_count} words over {raw_duration:.1f}s) — adjusting start")
                    start_sec = max(buffer[-1].end - 1.5, buffer[0].start)
                else:
